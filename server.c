...
static void handle_get (int connection_fd, const char* page, const char* args, int lenght)
{
 ...
 if (*(page-1) == '/' && strchr (page, '/') == NULL) {
    char module_file_name[64];
    
    /* The page name looks OK.  Construct the module name by appending
       ".so" to the page name.  */
    //lenght tem o tamanho do comando, e o +3 é para poder adicionar o .so
    snprintf(module_file_name, lenght, "%s", page);
    strcat(module_file_name, ".so");
    /* Try to open the module.  */
    module = module_open (module_file_name, NULL);
  }
  ...
  if (module == NULL) {
    ...
    (*module->generate_function) (connection_fd, args);
...
static void handle_connection (int connection_fd)
{
  ...
  if (bytes_read > 0) {
    ...
    //função que conta o numero de caracteres de uma string até a primeira ocorrencia
    ...
    if (strcmp (protocol, "HTTP/1.0") && strcmp (protocol, "HTTP/1.1")) {
	 ...
	 specified some other method, so report the failure.  */
      ...
      //Foi necessário copiar a string antes da ocorrencia do ?
      //caso contrário a função handle tentaria entrar em um diretório inexistente
      //__url contém o diretorio que se quer acessar
      // (url+args+1) contem a posição seguinte a que se encontra ao caracter "?"" se existir
      // args contem o tamanho (em letras) do diretorio que está guardado em __url
      handle_get (connection_fd,url+1, (url+args+1), args);
    }
  }
